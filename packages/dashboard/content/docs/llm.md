...
| `brainlayer` | Memory layer — external repo at `~/Gits/brainlayer/` (Python, 260K+ chunks, sqlite-vec) |
...
BrainLayer uses sqlite-vec with bge-large-en-v1.5 embeddings. For 260K+ chunks, the database is approximately 1.4GB on disk. Queries run in under 2 seconds. The embedding model loads into ~1.5GB of RAM when indexing, but the MCP server uses the pre-built index (no model loaded at query time).
...
| **Memory** | Python + sqlite-vec + sentence-transformers (BrainLayer — external repo) |
...
- **Tools:** Ralph (autonomous coding loop), BrainLayer (260K+ chunk memory layer with 10-field enrichment)
...
├── ~/Gits/brainlayer/               # Memory layer — external repo (Python + sqlite-vec)
...
Chose monolithic Python daemon over microservices. One process, one database, instant queries. BrainLayer now indexes 260K+ conversation chunks and returns results in under 2 seconds.
...
|--------|---------|-------|---------|
| **brainlayer** | `brainlayer-mcp` | 8 | Memory layer — search 260K+ indexed conversation chunks |
...
BrainLayer provides persistent memory across Claude Code sessions — semantic search over 260K+ indexed conversation chunks using bge-large-en-v1.5 embeddings (1024 dims) and sqlite-vec.
...
- **BrainLayer tools** query local sqlite-vec database (~1.4GB, 260K+ chunks)
...
BrainLayer is a **knowledge pipeline** that indexes every Claude Code conversation into a searchable database. It uses semantic embeddings to find past solutions, decisions, and patterns across all your projects. 260K+ chunks indexed, searchable in under 2 seconds.
...
[`~/Gits/brainlayer/`](https://github.com/EtanHey/brainlayer) (external repo)
