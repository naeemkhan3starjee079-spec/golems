...
|----------|-------------|---------|
| `RALPH_NTFY_PREFIX` | Prefix for per-project topics | `yourname-ralph` |
| `RALPH_NTFY_TOPIC` | Override topic (bypasses per-project) | (none) |
| `CLAUDE_NTFY_TOPIC` | Claude's notification topic | `yourname-claude-notify` |
...
```bash
# Per-project topics (default): yourname-ralph-{project}-notify
export RALPH_NTFY_PREFIX="yourname-ralph"

# Or override with fixed topic:
export RALPH_NTFY_TOPIC="my-custom-topic"

# Claude notifications (separate from Ralph):
export CLAUDE_NTFY_TOPIC="yourname-claude-notify"
```
