...
│   ├── default.sh        # Pattern A (Bash skills)
│   └── run.sh            # Pattern B (TypeScript skills — parses --action flag)
...
**Pattern A (Bash):** Simple CLI wrappers. `execute: scripts/default.sh`
**Pattern B (TypeScript):** Complex logic. `execute: scripts/run.sh --action=default` — run.sh forwards `--action` to the TS entry point which parses and dispatches on it.
