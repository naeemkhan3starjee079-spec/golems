  ...
  try {
    ...
    if (apiCalls > 0) {
      ...
      if (cutoffDate && firstTimestamp) {
        ...
        if (sessionDate < cutoffDate) return null;
      }
...
function friendlyProjectName(dirName: string): string {
  return dirName.replace(/^-Users-etanheyman-/, "").replace(/-/g, "/");
}

function projectNameFromPath(originalPath: string): string {
  return originalPath
    .replace(/^\/Users\/etanheyman\//, "")
    .replace(/^Desktop\/Gits\//, "Gits/")
    .replace(/^Gits\//, "");
}
...
function scanCCTranscripts(cutoffDate?: Date): SessionUsage[] {
  ...
  for (const proj of projectDirs) {
    ...
    const projectName = friendlyProjectName(proj);
  ...
  // Also scan archived sessions (~/.claude-archive/)
  const archiveDir = join(home, ".claude-archive");
  if (existsSync(archiveDir)) {
    const archiveSessions = scanArchiveTranscripts(
      archiveDir,
      cutoffDate,
      seenSessionIds,
    );
    sessions.push(...archiveSessions);
  }

  return sessions;
}

function scanArchiveTranscripts(
  archiveDir: string,
  cutoffDate?: Date,
  seenIds?: Set<string>,
): SessionUsage[] {
  ...
  // Structure: ~/.claude-archive/<projectId>/<archive-timestamp>/{manifest.json, *.jsonl}
  let topDirs: string[];
  try {
    topDirs = readdirSync(archiveDir);
  } catch {
    return [];
  }

  for (const projDir of topDirs) {
    const projPath = join(archiveDir, projDir);
    try {
      if (!statSync(projPath).isDirectory()) continue;
    } catch {
      continue;
    ...
    try {
      archiveDirs = readdirSync(projPath);
    } catch {
      continue;
    }

    for (const archDir of archiveDirs) {
      const archPath = join(projPath, archDir);
      try {
      ...
      const manifestPath = join(archPath, "manifest.json");
      try {
        if (existsSync(manifestPath)) {
          const manifest = JSON.parse(readFileSync(manifestPath, "utf-8"));
          if (manifest.originalPath) {
            projectName = projectNameFromPath(manifest.originalPath);
          }
        }
      } catch {
      ...
      // Parse JSONL files in this archive batch
      let files: string[];
      try {
        files = readdirSync(archPath).filter((f) => f.endsWith(".jsonl"));
      } catch {
        continue;
      }
      ...
      for (const file of files) {
        const sessionId = file.replace(".jsonl", "");
        if (seenIds?.has(sessionId)) continue; // skip duplicates

        const filePath = join(archPath, file);
        const result = parseSessionFile(
          filePath,
          sessionId,
          projectName,
...
function readHistory(): HistoryStats {
  ...
  const firstDate =
    firstTs < Infinity
      ...
      : "";
  const lastDate =
    lastTs > 0 ? new Date(lastTs * 1000).toISOString().slice(0, 10) : "";
...
async function fetchSupabaseCosts(period: string): Promise<SupabaseEntry[]> {
  ...
  try {
    const resp = await fetch(
      `${url}/rest/v1/llm_usage?select=created_at,model,cost_usd,tier,input_tokens,output_tokens${dateFilter}&order=created_at.desc&limit=10000`,
      {
        headers: {
          apikey: key,
...
function renderSummary(
  ...
  const ccInput = sessions.reduce(
    (s, x) => s + x.inputTokens + x.cacheReadTokens + x.cacheCreateTokens,
    0,
  );
  ...
  const localEntries = freeEntries.filter(
    (e) =>
      localModelNames.has(e.model) ||
      e.model.includes("glm") ||
      e.model.includes("qwen") ||
      e.model.includes("llama") ||
      e.model.includes("deepseek"),
  );
  const helperEntries = freeEntries.filter(
    (e) =>
      !localModelNames.has(e.model) &&
      !e.model.includes("glm") &&
      !e.model.includes("qwen") &&
  ...
  const estimatedExtraCalls = statsOnlyDays.reduce(
    (s, d) => s + Math.floor(d.messageCount / 2),
    0,
  );
  const estimatedExtraCost = estimatedExtraCalls * avgCostPerCall;

  console.log(
    `${c.bold}${c.magenta}Claude Code${c.reset} ${c.dim}($200/mo subscription)${c.reset}`,
  );
  console.log(
    `  Sessions: ${c.bold}${statsSessions > ccSessions ? statsSessions : ccSessions}${c.reset} ${c.dim}(${statsDays} active days)${c.reset}    Messages: ${c.bold}${statsMessages.toLocaleString()}${c.reset}`,
  );
  console.log(
    `  Tokens:   ${c.cyan}${formatTokens(ccInput)}${c.reset} in  ${c.yellow}${formatTokens(ccOutput)}${c.reset} out ${c.dim}(from ${ccSessions} sessions with transcript data)${c.reset}`,
  );
  if (estimatedExtraCost > 0) {
    console.log(
      `  Value:    ${costColor(ccCost)}${formatUSD(ccCost)}${c.reset} ${c.dim}(exact)${c.reset} + ${c.dim}~${formatUSD(estimatedExtraCost)} estimated (${statsOnlyDays.length} days without transcripts)${c.reset}`,
    );
    console.log(
      `  Saved:    ${c.green}~${formatUSD(Math.max(0, ccCost + estimatedExtraCost - 200))}${c.reset} ${c.dim}(value - $200 subscription)${c.reset}`,
    );
  } else {
    console.log(
      `  Value:    ${costColor(ccCost)}${formatUSD(ccCost)}${c.reset} ${c.dim}(at API rates)${c.reset}`,
    );
    console.log(
      `  Saved:    ${c.green}${formatUSD(Math.max(0, ccCost - 200))}${c.reset} ${c.dim}(value - $200 subscription)${c.reset}`,
    );
  }
  console.log();
  ...
  if (apiCalls > 0) {
    console.log(
      `${c.bold}${c.blue}Haiku API${c.reset} ${c.dim}(pay-per-use)${c.reset}`,
    );
    console.log(`  Calls:    ${c.bold}${apiCalls}${c.reset}`);
    console.log(
      `  Cost:     ${costColor(apiCost)}${formatUSD(apiCost)}${c.reset}`,
    );
    console.log();
  }
  ...
  console.log(
    `${c.bold}${c.yellow}Local Models${c.reset} ${c.dim}(Ollama — $0 cost)${c.reset}`,
  );
  if (localCalls > 0) {
    ...
    const modelList = Object.entries(byModel)
      ...
      .join(", ");
    console.log(
      `  Calls:    ${c.bold}${localCalls}${c.reset}    ${c.dim}${modelList}${c.reset}`,
    );
    if (localDurationSec > 0) {
      console.log(
        `  Runtime:  ${c.bold}${Math.round(localDurationSec / 60)}min${c.reset}`,
      );
    }
    console.log(
      `  Saved:    ${c.green}~${formatUSD(localSaved)}${c.reset} ${c.dim}(vs Haiku API)${c.reset}`,
    );
  } else {
    console.log(
      `  ${c.dim}No tracked calls yet — run email scorer or benchmarks to log${c.reset}`,
    );
  ...
  console.log(
    `${c.bold}${c.green}CLI Helpers${c.reset} ${c.dim}(gemini/cursor/codex/kiro — free)${c.reset}`,
  );
  if (helperCalls > 0) {
    ...
    const helperList = Object.entries(byHelper)
      ...
      .join(", ");
    console.log(
      `  Calls:    ${c.bold}${helperCalls}${c.reset}    ${c.dim}${helperList}${c.reset}`,
    );
    if (helperDurationSec > 0) {
      console.log(
        `  Runtime:  ${c.bold}${Math.round(helperDurationSec / 60)}min${c.reset}`,
      );
    }
    console.log(
      `  Saved:    ${c.green}~${formatUSD(helperEstimatedSaved)}${c.reset} ${c.dim}(vs Sonnet API)${c.reset}`,
    );
  } else {
    console.log(
      `  ${c.dim}No tracked calls yet — use helpers.ts functions to log${c.reset}`,
    );
    console.log(
      `  ${c.dim}Tracking enabled in helpers.ts (tier: "free")${c.reset}`,
    );
  }
  console.log();
  ...
  const totalSaved =
    ...
    200 * monthsActive +
    localSaved +
    helperEstimatedSaved;
  console.log(
    `${c.bold}  Total value:    ${costColor(totalValue + estimatedExtraCost)}${formatUSD(totalValue + estimatedExtraCost)}${c.reset}`,
  );
  console.log(
    `${c.bold}  Actual spend:   ${costColor(actualSpend)}${formatUSD(actualSpend)}${c.reset} ${c.dim}($200×${monthsActive}mo + ${formatUSD(apiCost)} API)${c.reset}`,
  );
  console.log(
    `${c.bold}  Total saved:    ${c.green}${formatUSD(Math.max(0, totalSaved))}${c.reset} ${c.dim}(CC sub + local + helpers)${c.reset}`,
  );
  const roi =
    totalValue + estimatedExtraCost > 0
      ? ((totalValue + estimatedExtraCost) / actualSpend).toFixed(1)
      : "0";
  ...
  if (period === "all") {
    ...
    if (history.totalSessions > 0) {
      console.log();
      console.log(
        `${c.bold}${c.gray}Full CC History${c.reset} ${c.dim}(from history.jsonl — no token data)${c.reset}`,
      );
      console.log(
        `  ${c.dim}Since: ${history.firstDate}  |  Total prompts: ${history.totalSessions.toLocaleString()}${c.reset}`,
      );
      const months = Object.entries(history.byMonth).sort();
      for (const [month, count] of months) {
        const bar = "█".repeat(Math.min(Math.round(count / 200), 40));
        console.log(
          `  ${c.dim}${month}: ${bar} ${count.toLocaleString()}${c.reset}`,
        );
      }
    }
  }
...
function renderDaily(sessions: SessionUsage[], apiEntries: SupabaseEntry[]) {
  ...
  console.log(
    `${c.dim}Date        CC Value    API Cost    CC Calls  API Calls  Tokens In    Tokens Out${c.reset}`,
  );
...
function renderByProject(sessions: SessionUsage[]) {
  ...
  for (const s of sessions) {
    ...
    projectMap[s.project].input +=
      s.inputTokens + s.cacheReadTokens + s.cacheCreateTokens;
    projectMap[s.project].output += s.outputTokens;
  }

  const projects = Object.entries(projectMap).sort(
  ...
  console.log(
    `${c.dim}${"Project".padEnd(maxName)}  Sessions  Calls  Value       Tokens In    Tokens Out${c.reset}`,
  );
...
function renderByModel(sessions: SessionUsage[]) {
  ...
  console.log(
    `${c.dim}${"Model".padEnd(maxName)}  Calls   Value       Tokens In    Tokens Out${c.reset}`,
  );
...
function renderJSON(
  ...
  const ccCost = sessions.reduce((s, x) => s + x.costUsd, 0);
  const ccCalls = sessions.reduce((s, x) => s + x.apiCalls, 0);
  const apiCost = apiEntries
    .filter((e) => e.tier === "paid")
  ...
  console.log(
    JSON.stringify({
      ...
      cc: {
        ...
        calls: ccCalls,
        valueUsd: Math.round(ccCost * 100) / 100,
      },
...
async function sendToAxiom(sessions: SessionUsage[]): Promise<void> {
  try {
    const { logCCUsage, flushAxiom } =
      await import("../packages/shared/src/lib/axiom");
    ...
    console.error(
      `${c.red}Failed to send to Axiom:${c.reset}`,
...
async function main() {
  ...
  const period =
    args.find((a) => a.startsWith("--period="))?.split("=")[1] || "month";
