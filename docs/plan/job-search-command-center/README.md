...
| 7 | Cost Tracking + Observability | [phase-7](phase-7/) | done | `feature/jscc-phase7-observability` | Supabase persistence, CC usage via ccusage, MCP tools, value metrics |
...
- **Interview Practice with Memory:** RecruiterGolem runs interactive leetcode/system-design/behavioral sessions with persistent Elo tracking per topic, session history in Supabase (`practice_sessions` + `practice_questions` tables already exist), weak area detection, and adaptive difficulty. The `/interview-practice` skill template exists but needs RecruiterGolem integration for state + memory.
- **Dashboard Reliability:** `service_runs` table is empty — `reportServiceRun()` writes to `golem_state` instead. Dashboard likely reads `service_runs`. Fix: either populate `service_runs` properly or update dashboard to read `golem_state`. Also: Greenhouse/Lever scrapers (Phase 8) not yet wired into cloud-worker schedule.
