...
7. **Verify docs rendering parity** — Side-by-side comparison of every doc page in dashboard vs portfolio. Same padding, syntax highlighting, typography. Screenshot comparison.
8. **Verify type centralization** — No inlined types remaining in any dashboard page component. All in `src/lib/types/`.
9. **Verify data pages populated** — Emails, recruiter, teller, jobs pages all show real data (not empty). Fix any query/rendering bugs.
10. **Final build test** — Dashboard builds, docs render correctly, all pages accessible.
11. **Commit + PR** — Single PR for entire docs verification pass.
...
- [ ] Verify docs rendering parity (dashboard = portfolio)
- [ ] Verify type centralization (no inlined types)
- [ ] Verify data pages populated (emails, recruiter, teller, jobs)
- [ ] Final build test
- [ ] Commit + PR
